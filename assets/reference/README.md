...
directory. Export with `just export-currencies <app-path>` from that checkout, or
copy its versioned `reference/currencies/` artifact here. Run `just check` here to
verify this independent copy through the shared pinned `currency --app-check`
command. Do not hand-edit generated files. The App has no runtime dependency on
